(
  function() {
    ...
    angular
      ...
      .component('authorizerAddList', authorizerAddList());

    function authorizerAddList() {
      var cdo = {
        templateUrl: 'authorizer-add-list.template.html',
        bindings: {
          inputId: '@',
          model: '=ngModel',
          disabled: '=ngDisabled',
        ...
        controller: AuthorizerAddListController
      };

      /* @ngInject */
      function AuthorizerAddListController($q, $document, $timeout, RequisitionAuthorizerApiService) {

        var self = this;

        self.$onInit = function() {
          if (_.isNil(self.inputId)) {
            self.inputId = _.uniqueId('authorizerAddList_');
          }
        ...
        self.$onChanges = function(changes) {
          ...
          if (isDirty(changes.requisitionId)) {
            self.collection = [];
            self.fetchFromDatabase = true;

            if (isNull(changes.requisitionId)) {
        ...
        function initPaging(pageOffset, searchText) {
          var result = {
            ...
            authorizerToSubstituteId: self.authorizerToSubstitute ? self.authorizerToSubstitute.id : null,
