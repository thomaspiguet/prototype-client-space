(
  function() {
    ...
    angular
      .module('app.controls.class-list')
      .component('classList', classList());

    function classList() {
      var cdo = {
        templateUrl: 'class-list.template.html',
        bindings: {
          model: '=ngModel',
          inputId: '@',
          disabled: '=ngDisabled',
      ...
      /*@ngInject*/
      function ClassListController($q, $timeout, ClassificationApiService) {
        var self = this;
        self.searchText = '';
        self.collection = [];
        ...
        self.$onChanges = function(changes) {
            ...
            if (isDirty(changes.segment) || isDirty(changes.family)) {
                self.collection = [];
                fetchFromDatabase = true;

                if (isNull(changes.segment) || isNull(changes.family)) {
                    self.searchText = '';
        ...
        self.getClasses = function() {
          ...
          if (fetchFromDatabase && !_.isNil(self.segment) && !_.isNil(self.family)) {
            var params = {
              sortBy: ['code']
            };
            ...
            params.expanded = false;
              ...
              .getClasses(params, self.segment.code, self.family.code)
              .then(
                ...
                function failure(error) {
                  deferred.reject('Problem getting Class data [' + error + ']');
                }
              )
            ;
          }
